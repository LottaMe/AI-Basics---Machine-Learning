...
def find_maximum():
    ...
    # evaluate initial state (0,0)?
    # loop until optimum goes down again
    # assign randomly at start
    toast_duration = 3
    wait_duration = 90
    ...
    while True:
        ...
        next_solution = get_max_neighbor(*optimum)
        if (utility(*optimum) < utility(*next_solution)):
            optimum = next_solution
        else:
            break
...
def get_max_neighbor(x, y):
    neighbors = get_all_neighbors(x, y)
    maxi = neighbors[0]
    for neighbor in neighbors:
        if utility(*maxi) <= utility(*neighbor):
            maxi = neighbor
    return maxi 
