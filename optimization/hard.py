...
def find_maximum():
    # TODO: Implement an optimization algorithm. Tip: (1,1) is not the optimum!hard: 
#    - also use the parameter power
#    - e.g., utility(2,3,1.2) float is between 0 and 2
#    - this introduces the following complications:
#        - multiple maxima
#        - a continuous parameter
#    - implement gradient ascent
    current_solution = (0, 0, 0.0)
# how to define the gradient - defining step-size
# first two parameters are still defined by the hill-climbing
    while True:
...
def get_gradient(state):
    # state = point to evaluate the gradient at
    # utility = function that takes *state as argument
    current_gradient = utility(*state)
