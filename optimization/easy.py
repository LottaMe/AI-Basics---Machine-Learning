...
def find_maximum():
    ...
    # toast_duration is an int between 1 and 100
    # wait_duration is an int between 1 and 100
    # return a tuple with (toast_duration, wait_duration)
    # that tuple should lead to the highest value for utility(*(toast_duration, wait_duration))
    toast_duration = 0
    wait_duration = 0
    optimum = (toast_duration, wait_duration)
    for toast_duration in range(1, 100):
        for wait_duration in range(1, 100):
            if utility(toast_duration, wait_duration) > utility(*optimum):
                optimum = (toast_duration, wait_duration)
    return (optimum)
